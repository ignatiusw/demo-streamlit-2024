...
WITH CTE_TARGET_MONTH AS (
    SELECT REGION, DATEADD(MONTH, 1, MAX(MONTH)) AS MONTH, 1 AS ADD_MONTHS
    FROM DEMO_PROGRAMMABLE_2024_STREAMLIT_DB.DATA.DEMO_DATA
    WHERE GROSS_SALE IS NOT NULL AND SALES_COST IS NOT NULL
    GROUP BY REGION
    UNION ALL
    SELECT REGION, DATEADD(MONTH, 1, MONTH) AS MONTH, 1 + ADD_MONTHS AS ADD_MONTHS
    FROM CTE_TARGET_MONTH
    WHERE ADD_MONTHS <= 11
)
SELECT t.REGION, t.MONTH, d.TARGET_REVENUE, CAST(NULL AS TIMESTAMP_TZ) AS UPDATED_TS, CAST(NULL AS VARCHAR(200)) AS UPDATED_BY
FROM CTE_TARGET_MONTH t
LEFT JOIN DEMO_PROGRAMMABLE_2024_STREAMLIT_DB.DATA.DEMO_DATA d
...
AS
SELECT CAST(MONTH AS TIMESTAMP_NTZ) AS MONTH, REGION, GROSS_SALE - SALES_COST AS REVENUE, TARGET_REVENUE
FROM DEMO_PROGRAMMABLE_2024_STREAMLIT_DB.DATA.DEMO_DATA
WHERE GROSS_SALE IS NOT NULL AND SALES_COST IS NOT NULL;
